...
# Copyright (C) 1988, 1998, 2000, 2002, 2004-2005, 2007-2014 Free
...
# Author    : Harel Ben Attia - harelba@gmail.com, harelba @ github,
# @harelba on twitter
...
=======
  ...
  STDOUT = codecs.getwriter(locale.getpreferredencoding())(sys.stdout)
>>>>>>> 6817f8ceefa37d00785795cc4a2e50f141758995
...
<<<<<<< HEAD
                      ...
                      Its purpose is to bring SQL expressive power to
                      manipulating text data using the Linux command line.

                      Basic usage is q "<sql like query>" where table names are
                      just regular file names (Use - to read from standard
                      input)
    
                      Columns are named c1..cN and delimiter can be set using
                      the -d (or -t) option.
                      ...
                      group by c1"
                      This example would print a count of each unique
                      permission string in the current folder.

                      Example 2: seq 1 1000 | q "select avg(c1),sum(c1) from -"
                      This example would provide the average and the sum of the
                      numbers in the range 1 to 1000

                      Example 3: sudo find /tmp -ls | q "select
                      c5,c6,sum(c7)/1024.0/1024 as total from - group by c5,c6
                      order by total desc"
                      This example will output the total size in MB per
                      user+group in the /tmp subtree

                      See the help or https://github.com/harelba/q for more
                      details.
                      """)
parser.add_option("-b", "--beautify", dest="beautify",
                  default=default_beautify, action="store_true",
                  help="Beautify output according to actual values. Might be\
                  slow...")
parser.add_option("-z", "--gzipped", dest="gzipped", default=default_gzipped,
                  action="store_true",
                  help="Data is gzipped. Useful for reading from stdin. For\
                  files, .gz means automatic gunzipping")
parser.add_option("-d", "--delimiter", dest="delimiter",
                  default=default_delimiter,
                  help="Field delimiter. If none specified, then space is used\
                  as the delimiter. If you need multi-character delimiters, run\
                  the tool with engine version 1 by adding '-E v1'. Using v1\
                  will also revert to the old behavior where if no delimiter is\
                  provided, then any whitespace will be considered as a\
                  delimiter.")
parser.add_option("-D", "--output-delimiter", dest="output_delimiter",
                  default=default_output_delimiter,
                  help="Field delimiter for output. If none specified, then the\
                  -d delimiter is used if present, or space if no delimiter is\
                  specified")
parser.add_option("-t" , "--tab-delimited-with-header",\
                  dest="tab_delimited_with_header", default=False,\
                  action="store_true", help="Same as -d <tab> -H 1. Just a\
                  shorthand for handling standard tab delimited file with one\
                  header line at the beginning of the file")
parser.add_option("-H", "--header-skip", dest="header_skip",
                  default=default_header_skip,
                  help="Skip n lines at the beginning of the data (still takes\
                  those lines into account in terms of structure)")
parser.add_option("-f", "--formatting", dest="formatting",
                  default=default_formatting,
                  help="Output-level formatting, in the format X=fmt,Y=fmt etc,\
                  where X,Y are output column numbers (e.g. 1 for first SELECT\
                  column etc.")
parser.add_option("-e", "--encoding", dest="encoding",
                  default=default_encoding,
                  help="Input file encoding. Defaults to UTF-8. set to none for\
                  not setting any encoding - faster, but at your own risk...")
parser.add_option("-v", "--version", dest="version", default=False,
                  action="store_true", help="Print version")
parser.add_option("-E", "--engine-version", dest="engine_version",
                  default='v2',
                  help="Engine version to use. v2 is the default, and supports\
                  quoted CSVs, but requires a specific delimiter (can't use\
                  multi-character delimiters). Use v1 if you need multi-char\
                  delimiters or if you encounter any problems (and please\
                  notify me if you do)")
=======
    q allows performing SQL-like statements on tabular text data.
...
<<<<<<< HEAD
    ...
    def _get_as_list_str(self, l, quote=False):
        if not quote:
            ...
            # Unfortunately, using list comprehension and performing
            # replace on all elements here slows things down, so this
            # is an optimization.
    ...
    def generate_insert_row(self, table_name, column_names, col_vals):
        ...
        return 'INSERT INTO %s (%s) VALUES (%s)' % (table_name, col_names_str,
                                                    col_vals_str)
    ...
    # Get a list of column names so order will be preserved.
    # (Could have used OrderedDict, but then we would need python 2.7)
    def generate_create_table(self, table_name, column_names, column_dict):
        # Convert dict from python types to db types
        column_name_to_db_type = dict((n, self.type_names[t]) for n,t in
        column_dict.iteritems())
        column_defs = ','.join(['%s %s' % (n, column_name_to_db_type[n]) for n
                                           in column_names])
...
# Simplistic Sql "parsing" class... We'll eventually require a real SQL
# parser which will provide us with a parse tree.
#
...
<<<<<<< HEAD
    def __init__(self, sql):
        ...
        # Dict from qtable names to their positions in sql_parts.  Value
        # here is a *list* of positions, since it is possible that the 
        # same qtable_name (file) is referenced in multiple positions
        # and we don't want the database table to be recreated for each
        # reference
        self.qtable_name_positions = {}
        # Dict from qtable names to their effective (actual database)
        # table names
        ...
        while idx < len(self.sql_parts):
            ...
            if part.upper() in ['FROM', 'JOIN']:
                ...
                if idx == len(self.sql_parts)-1:
                    # Just fail
                    raise Exception('FROM/JOIN is missing a table name after\
                                    it')
                qtable_name = self.sql_parts[idx + 1]
                # Otherwise, the next part contains the qtable name. In
                # most cases the next part will be only the qtable name.
                # We handle one special case here, where this is a
                # subquery as a column:
                # "SELECT (SELECT ... FROM qtable),100 FROM ...".
                # In that case, there will be an ending paranthesis as
                # part of the name, and we want to handle this case
                # gracefully.  This is obviously a hack of a hack :) Just
                # until we have complete parsing capabilities
                if ')' in qtable_name:
    ...
    def set_effective_table_name(self, qtable_name, effective_table_name):
        ...
        if qtable_name in self.qtable_name_effective_table_names.keys():
            raise Exception("Already set effective table name for qtable %s" %
                            qtable_name)
        self.qtable_name_effective_table_names[qtable_name] =
        effective_table_name

    def get_effective_sql(self):
        if len(filter(lambda x: x is None,
        self.qtable_name_effective_table_names)) != 0:
...
class TableColumnInferer(object):
    ...
    def analyze(self, example_col_vals):
        ...
        if self.column_count == 1:
            print >>sys.stderr,"Warning: column count is one - did you provide\
                                the correct delimiter?"
        if self.column_count == 0:
            raise Exception("Detected a column count of zero... Failing")
        # FIXME: Hack to provide for some small variation in the column
        # count.  Will be fixed as soon as we have better column
        # inferring
...
class TableCreator(object):
    def __init__(self, db, filenames_str, line_splitter, header_skip=0,
                 gzipped=False, encoding='UTF-8', file_reading_method='manual'):
        self.db = db
        ...
        # Filled only after table population since we're inferring the
        # table creation data
        ...
        for fileglob in filenames:
            ...
            for filename in files_to_go_over:
                ...
                if not self.table_created:
                    raise Exception('Table should have already been created for\
                                    file %s' % filename)
    ...
    def _insert_row(self, col_vals):
        ...
        # If the table is still not created, then we don't have enough
        # data, just return
        if not self.table_created:
            return
    ...
    def _insert_row_i(self, col_vals):
        ...
        if len(col_vals) > len(self.column_inferer.column_names):
            err_text = 'Encountered a line in an invalid format %s:%s - %s\
            columns instead of %s. Did you make sure to set the correct\
            delimiter?' %(self.current_filename,self.lines_read, len(col_vals),
            len(self.column_inferer.column_names))
            raise ColumnCountMismatchException(err_text)
        effective_column_names =
            self.column_inferer.column_names[:len(col_vals)]
    ...
    def _flush_inserts(self):
        # print
        # self.db.execute_and_fetch(self.db.generate_begin_transaction())
        for col_names, col_vals in self.buffered_inserts:
            insert_row_stmt = self.db.generate_insert_row(self.table_name,
                                                          col_names, col_vals)
            self.db.execute_and_fetch(insert_row_stmt)
        # print
        # self.db.execute_and_fetch(self.db.generate_end_transaction())
        self.buffered_inserts = []
    ...
    def try_to_create_table(self, col_vals):
        ...
        if result:
            ...
            create_table_stmt = self.db.generate_create_table(self.table_name,
                self.column_inferer.get_column_names(), column_dict)
            # And create the table itself
            self.db.execute_and_fetch(create_table_stmt)
            # Mark the table as created
            self.table_created = True
        else:
            pass # We don't have enough information for creating the 
                 # table yet
...
# If the user flagged for a tab-delimited file then set the delimiter to
# tab
...
<<<<<<< HEAD
    if options.delimiter is None:
        ...
        print >>sys.stderr, "Delimiter must be one character only. Add '-E v1'\
                             to the command line if you need multi-character\
                             delimiters. This will revert to version 1 of the\
                             engine which supports that. Please note that v1\
                             does not support quoted fields."
        sys.exit(5)
    q_dialect = {'skipinitialspace': True, 'quoting': 0, 'delimiter':
                 options.delimiter, 'quotechar': '"', 'doublequote': False}
...
try:
    ...
    for filename in sql_object.qtable_names:
        ...
        table_creator = TableCreator(db, filename, line_splitter,
                                     int(options.header_skip),
                                     options.gzipped, options.encoding,
                                     file_reading_method)
    ...
    print >>sys.stderr, "Cannot decode data. Try to change the encoding by\
                         setting it using the -e parameter. Error:%s" % e
    sys.exit(3)
...
if options.output_delimiter:
    ...
    if options.delimiter:
        # If an input delimiter is specified, then we use it as the
        # output as well.
        output_delimiter = options.delimiter
    else:
        # If no input delimiter is specified, then we use space as the
        # default (since no input delimiter means any whitespace).
        output_delimiter = " "
if options.formatting:
<<<<<<< HEAD
    formatting_dict = dict([(x.split("=")[0], x.split("=")[1]) for x in
                           options.formatting.split(",")])
=======
...
<<<<<<< HEAD
    for rownum,row in enumerate(m):
        ...
        for i, col in enumerate(row):
            if formatting_dict is not None and str(i + 1) in
            formatting_dict.keys():
